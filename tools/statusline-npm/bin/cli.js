...
function checkDependencies() {
	...
	if (jqCheck.status !== 0) {
		console.error(
			`${colors.red}❌ Error: jq is required but not installed.${colors.reset}`,
		);
		console.error(`   ${colors.cyan}Install with:${colors.reset}`);
		console.error(
			`     sudo apt install jq      ${colors.yellow}# Ubuntu/Debian${colors.reset}`,
		);
		console.error(
			`     brew install jq          ${colors.yellow}# macOS${colors.reset}`,
		);
		console.error(
			`     yum install jq           ${colors.yellow}# RHEL/CentOS${colors.reset}`,
		);
		process.exit(1);
	}
	...
	if (gitCheck.status !== 0) {
		console.warn(
			`${colors.yellow}⚠️  Warning: git not found. Git info will not be available.${colors.reset}`,
		);
	}
}
	...
	switch (command) {
		...
		default:
			if (command) {
				...
				checkDependencies();
				const defaultInstallResult = spawnSync(
					"bash",
					[path.join(scriptsDir, "install.sh")],
					{
						stdio: "inherit",
