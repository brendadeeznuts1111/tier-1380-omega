...
import {
	CRC32Performance,
	OMEGACRC32,
} from "./core/performance/CRC32Performance.ts";
...
urlHashes.forEach(({ url, hash }) => {
	console.log(
		`${hash.toString(16).padStart(8, "0")}: ${url.substring(0, 60)}...`,
	);
});
...
console.log(
	"- Support for all S3 operations (reads, writes, stat, multipart uploads)",
);
