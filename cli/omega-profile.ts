...
import { profilePublisher, generateProfileUrlCli, parseProfileUrlCli, generateSequenceCli } from '../core/profile/ProfilePublisher.ts';
...
class ProfileCLI {
  ...
  private async generateProfile(args: string[]): Promise<void> {
    ...
    if (!options.type) {
      console.error('❌ Profile type required (cpu, heap, tension, memory, performance)');
      return;
    }
    ...
    const profileUrl = profilePublisher.generateProfileUrl({
      ...
      timestamp: options.timestamp ? parseInt(options.timestamp, 10) : Date.now(),
      team: options.team,
  ...
  private async parseProfile(args: string[]): Promise<void> {
    ...
    if (!metadata) {
      console.error('❌ Invalid profile URL or does not follow Tier-1380 OMEGA convention');
      return;
    }
  ...
  private async generateSequence(args: string[]): Promise<void> {
    ...
    const startTimestamp = options.timestamp ? parseInt(options.timestamp, 10) : Date.now();
  ...
  private async getLatest(args: string[]): Promise<void> {
    ...
    const latest = await profilePublisher.getLatestProfile(type as any, environment as any);
    
    if (!latest) {
      console.log(`❌ No latest ${type} profile found for ${environment} environment`);
      return;
    }
    ...
    console.log(`Timestamp: ${new Date(latest.metadata.timestamp).toISOString()}`);
  }
  ...
  private async validateProfile(args: string[]): Promise<void> {
    ...
    if (isValid) {
      ...
      console.log('Expected pattern: https://profiles.factory-wager.com/{type}/{tier}/{environment}/{timestamp}_{type}-{timestamp}.{format}');
    }
  }
