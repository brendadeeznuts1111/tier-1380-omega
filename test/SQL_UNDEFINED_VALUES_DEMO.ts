...
class MockSQLHelper {
  static sql(strings: TemplateStringsArray, ...values: any[]): { text: string; values: any[] } {
    let text = '';
    const params: any[] = [];
    let paramIndex = 1;
  ...
  private static buildInsertQuery(data: Record<string, any>, startParamIndex: number): { text: string; values: any[] } {
...
export class SQLUndefinedValueDemo {
  ...
  demonstrateUndefinedValueHandling(): void {
    ...
    console.log('❌ Problem: undefined values become NULL, overriding database DEFAULTs');
    console.log('❌ Result: team = NULL, benchmark = NULL (instead of DEFAULT values)\n');
    ...
    console.log('✅ Result: team, benchmark, metadata columns omitted entirely');
    console.log('✅ Database uses DEFAULT values: team = \'unassigned\', benchmark = \'baseline\'\n');

    // Actually execute to show the result
    this.db.run(query.text, ...query.values);
    const result = this.db.query('SELECT * FROM profiles WHERE id = ?', ['cpu-profile-new']).get();
  ...
  demonstrateBulkInsertWithMixedUndefined(): void {
    ...
    // Show results
    const results = this.db.query('SELECT id, type, team, benchmark, metadata FROM profiles WHERE id LIKE "bulk-profile-%"').all();
    console.log('\n📊 Bulk insert results:');
    results.forEach((row: any) => {
      console.log(`- ${row.id}: team=${row.team}, benchmark=${row.benchmark}, metadata=${row.metadata}`);
    });
  ...
  demonstrateDataLossBugFix(): void {
    ...
    const results = this.db.query('SELECT id, team, benchmark, metadata FROM profiles WHERE id LIKE "fix-demo-%"').all();
  ...
  demonstratePracticalUseCases(): void {
    ...
    const query1 = this.sql`INSERT INTO "profiles" ${this.sql(optionalMetadata)}`;
  ...
  runCompleteDemo(): void {
    ...
    console.log('📖 Bun v1.3.6 Feature: sql() INSERT helper now respects undefined values\n');
