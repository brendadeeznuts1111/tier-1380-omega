...
import { describe, it, expect, test, beforeAll, afterAll, beforeEach, afterEach } from "bun:test";
  ...
  describe("should handle undefined values", () => {
    ...
    it("should handle bulk inserts with mixed undefined values", () => {
      const profiles = [
        ...
        OMEGATestUtils.generateTestProfile({ team: 'runtime', benchmark: 'test-1' }),
        OMEGATestUtils.generateTestProfile({ team: 'compiler', benchmark: undefined })
      ];
...
describe("CRC32 Performance", () => {
  describe("should handle hardware acceleration", () => {
    ...
    it("should verify profile integrity", () => {
      ...
      expect(CRC32Performance.verifyProfileIntegrity(data, hash + 1)).toBe(false);
    });
  });
