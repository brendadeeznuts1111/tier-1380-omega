...
async function main() {
	...
	console.log(
		`   Auto-expires: ${vectorStore.expires_after?.days} days after last use\n`,
	);
...
- Critical incidents escalated within 1 hour`,
	...
	const filesInStore = await openai.beta.vectorStores.files.list(
		vectorStore.id,
	);
	...
	for (const store of allStores.data) {
		console.log(
			`      - ${store.name} (${store.id}): ${store.file_counts.completed} files`,
		);
	}
	...
	console.log(
		`      await openai.beta.vectorStores.files.del("${vectorStore.id}", "file_id");`,
	);
	console.log("");
	...
	console.log("");
	console.log(
		"   Note: Vector store will auto-delete after 30 days of inactivity (configured above)",
	);
