...
async function sendMessage(
	...
	if (runStatus.status !== "completed") {
		throw new Error(
			`Run ${runStatus.status}: ${runStatus.last_error?.message}`,
		);
	}
	...
	for (const [userId, threadId] of userThreads.entries()) {
		try {
			...
			if (ageHours > maxAgeHours) {
				...
				deletedCount++;
				console.log(
					`   ✅ Deleted thread for user ${userId} (age: ${ageHours.toFixed(1)}h)`,
				);
			}
		} catch (error) {
...
async function main() {
	...
	let response1 = await sendMessage(
		thread1,
		assistant.id,
		"Hello! What's 2+2?",
	);
	...
	const thread2 = await getOrCreateUserThread(user2);
