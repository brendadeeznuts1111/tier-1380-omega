...
export class Batcher extends DurableObject<Env> {
	...
	async addItem(item: string): Promise<void> {
		...
		if (currentAlarm === null) {
			...
			console.log(
				`Alarm scheduled for ${new Date(Date.now() + 10000).toISOString()}`,
			);
		} else {
			console.log(
				`Alarm already scheduled for ${new Date(currentAlarm).toISOString()}`,
			);
		}
	}
	...
	async alarm(alarmInfo: AlarmInfo): Promise<void> {
		console.log(
			`Alarm fired (retry: ${alarmInfo.isRetry}, count: ${alarmInfo.retryCount})`,
		);
...
export class ReminderDO extends DurableObject {
	async setReminder(message: string, fireAt: Date): Promise<void> {
		// Store reminder data
		await this.ctx.storage.put("reminder", {
			message,
	...
	async alarm(): Promise<void> {
		const reminder = await this.ctx.storage.get<{
			message: string;
			fireAt: number;
		}>("reminder");
