 ...
 */
export async function DONT_useTraditionalTransaction(
...
export async function batchMixedOperations(db: ReturnType<typeof drizzle>) {
	const results = await db.batch([
		...
		db
			.delete(comments)
			.where(lt(comments.createdAt, new Date("2024-01-01")))
			.returning(),
	]);
...
export async function batchReadOperations(
	db: ReturnType<typeof drizzle>,
	userIds: number[],
) {
