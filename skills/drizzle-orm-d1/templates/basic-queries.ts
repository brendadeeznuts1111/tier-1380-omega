...
export async function unpublishOldPosts(
	db: ReturnType<typeof drizzle>,
	cutoffDate: Date,
) {
	return await db
		...
		.where(and(eq(posts.published, true), lt(posts.createdAt, cutoffDate)))
		.returning();
}
...
export async function deletePostsByAuthor(
	db: ReturnType<typeof drizzle>,
	authorId: number,
) {
	return await db.delete(posts).where(eq(posts.authorId, authorId)).returning();
}
...
export async function deleteUnpublishedPostsOlderThan(
	...
	return await db
		.delete(posts)
		.where(and(eq(posts.published, false), lt(posts.createdAt, cutoffDate)))
		.returning();
}
