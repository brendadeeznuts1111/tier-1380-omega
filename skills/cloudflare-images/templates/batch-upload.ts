...
export async function uploadMultipleImages(
	...
	Array<{ input: unknown; result?: CloudflareImagesResponse; error?: string }>
> {
	const results: Array<{
		input: unknown;
	...
	for (const chunk of chunks) {
		const promises = chunk.map(async (img) => {
			try {
				...
				if (img.file) {
					result = await batchUploadImage(
						img.file,
						{ id: img.id, metadata: img.metadata },
						env,
					);
				} else if (img.url) {
					result = await batchUploadViaURL(
						img.url,
						{ id: img.id, metadata: img.metadata },
						env,
					);
