...
const creativeWriterAgent = new Agent({
	...
	instructions:
		"Write engaging, creative marketing copy with emotional appeal and storytelling.",
	outputType: z.object({
		headline: z.string(),
		body: z.string(),
		cta: z.string(),
...
const technicalWriterAgent = new Agent({
	...
	instructions:
		"Write clear, factual marketing copy focused on features and benefits.",
	outputType: z.object({
		headline: z.string(),
		body: z.string(),
		cta: z.string(),
...
const humorWriterAgent = new Agent({
	name: "Humor Writer",
	instructions:
		"Write fun, witty marketing copy that entertains while informing.",
	outputType: z.object({
		headline: z.string(),
		body: z.string(),
		cta: z.string(),
...
async function generateMarketingCopyVariants(product: string) {
	...
	console.log(
		"Total:",
		creative.usage.totalTokens +
			technical.usage.totalTokens +
			humor.usage.totalTokens,
	);
...
const evaluatorAgent = new Agent({
	name: "Evaluator",
	instructions:
		"Rate the quality of this marketing copy on a scale of 1-10 and explain why.",
	outputType: z.object({
...
async function selectBestVariant(product: string) {
	...
	const evaluations = await Promise.all([
		run(
			evaluatorAgent,
			`Evaluate this headline: ${variants.creative.finalOutput.headline}`,
		),
		run(
			evaluatorAgent,
			`Evaluate this headline: ${variants.technical.finalOutput.headline}`,
		),
		run(
			evaluatorAgent,
			`Evaluate this headline: ${variants.humor.finalOutput.headline}`,
		),
	]);
