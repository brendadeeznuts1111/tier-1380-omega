...
async function createFileSearchStore(options: CreateStoreOptions) {
	...
	try {
		...
		do {
			const page = await ai.fileSearchStores.list({
...
function parseArgs(): CreateStoreOptions {
	...
	if (!options.name) {
		console.error(
			'Usage: ts-node create-store.ts --name "Store Name" [--project "project"] [--environment "env"]',
		);
		console.error("\nExample:");
		console.error(
			'  ts-node create-store.ts --name "Customer Support KB" --project "support" --environment "production"',
		);
		process.exit(1);
	}
...
export { createFileSearchStore, type CreateStoreOptions };
