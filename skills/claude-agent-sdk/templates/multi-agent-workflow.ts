...
const appTools = createSdkMcpServer({
	...
	tools: [
		...
		tool(
			...
			async (args) => {
				console.log(
					`📝 [${args.severity.toUpperCase()}] ${args.event}:`,
					args.data || "",
				);
				return {
...
Follow the principle of least privilege for tool access.`,
			...
			agents: {
				"deployment-agent": {
					description:
						"Handles application deployments, rollbacks, and release management",
...
Always notify stakeholders of deployment status.`,
				...
				"incident-responder": {
					description:
						"Responds to production incidents, outages, and performance issues",
...
Work quickly but carefully. User experience is critical.`,
				...
				"monitoring-agent": {
					description:
						"Monitors system metrics, health checks, and alerts on issues",
