...
async function comprehensiveErrorHandling() {
	...
	try {
		...
		for await (const message of response) {
			switch (message.type) {
				...
				case "error":
					...
					if (message.error.type === "permission_denied") {
						console.log(
							"→ Permission was denied, continuing with limited access",
						);
		...
		if (error.code === "AUTHENTICATION_FAILED") {
			...
			console.error(
				"→ Consider using session management or reducing prompt size",
			);
		...
		if (errors.length > 0) {
			...
			errors.forEach((err) => {
				console.log(
					`${err.timestamp.toISOString()} - ${err.type}: ${err.message}`,
				);
			});
		}
	}
...
class CircuitBreaker {
	...
	private onFailure() {
		...
		console.warn(
			`Circuit breaker: ${this.failures}/${this.threshold} failures`,
		);
	}
}
