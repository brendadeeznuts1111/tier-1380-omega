...
async function cacheConversationHistory() {
	const messages: Anthropic.MessageParam[] = [
		...
		{
			role: "assistant",
			content: "Interfaces in TypeScript define contracts...",
		},
		{ role: "user", content: "Can you give examples?" },
	];

	// Cache the conversation history
	const messagesWithCache: Anthropic.MessageParam[] = messages
		.slice(0, -1)
		.map((msg, idx) => {
...
function calculateCachingSavings(
	...
	const cacheReadCost =
		(inputTokens / 1_000_000) * cacheReadCostPerMTok * (turnCount - 1); // Subsequent
	...
	console.log("\n=== Cost Comparison ===");
	console.log(
		`Input tokens: ${inputTokens}, Output tokens: ${outputTokens}, Turns: ${turnCount}`,
	);
