...
async function compareThinkingModes() {
	...
	console.log(
		"=== Standard Response (Sonnet 4.5 - no extended thinking) ===\n",
	);
	...
	console.log(
		"\nTokens used:",
		response1.usage.input_tokens + response1.usage.output_tokens,
	);
...
async function demonstrateWrongModelError() {
	try {
		...
		if (!hasThinking) {
			...
			console.log(
				"Extended thinking is only available in Opus 4.5, Sonnet 4.5, and Opus 4.",
			);
		}
