...
async function detailedPrompts() {
	...
	const artStyle = await openai.images.generate({
		model: "dall-e-3",
		prompt:
			"An oil painting of a sunset over the ocean in the style of Claude Monet",
		style: "natural",
	});
	...
	const detailed = await openai.images.generate({
		model: "dall-e-3",
		prompt:
			"A professional product photo of a smartwatch on a white marble surface, with soft studio lighting from the left, shallow depth of field, commercial photography style",
		quality: "hd",
		style: "natural",
	});
	...
	const character = await openai.images.generate({
		model: "dall-e-3",
		prompt:
			"A friendly robot character with round edges, bright blue and white colors, large expressive eyes, modern minimalist design, 3D render style",
		style: "vivid",
	});
