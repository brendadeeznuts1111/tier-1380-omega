...
async function classifyText() {
	const text =
		"This product is absolutely terrible. It broke after one day of use. Very disappointed!";
		...
		messages: [
			{
				role: "system",
				content:
					"Classify the sentiment and extract key information from product reviews.",
			},
			{
				role: "user",
				content: text,
...
async function simpleJsonMode() {
	const completion = await openai.chat.completions.create({
		...
		messages: [
			{
				role: "user",
				content: "List 3 programming languages and their use cases as JSON",
			},
		],
...
async function withEnums() {
	const completion = await openai.chat.completions.create({
		...
		messages: [
			{
				role: "user",
				content: "Categorize this as a bug report: The app crashes on startup",
			},
		],
		response_format: {
			type: "json_schema",
	...
	for (const text of texts) {
		const completion = await openai.chat.completions.create({
			...
			messages: [
				{
					role: "system",
					content: "Extract key information as structured data",
				},
