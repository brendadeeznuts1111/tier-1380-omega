...
async function voiceInstructions() {
	...
	const professional = await openai.audio.speech.create({
		...
		instructions:
			"Speak in a calm, professional, and friendly tone suitable for customer service.",
	});
	...
	const energetic = await openai.audio.speech.create({
		...
		instructions:
			"Use an enthusiastic, energetic tone perfect for marketing and advertisements.",
	});
	...
	const soothing = await openai.audio.speech.create({
		...
		voice: "shimmer",
		input:
			"Take a deep breath. Relax your shoulders. Let all tension fade away.",
		instructions:
			"Adopt a calm, soothing voice suitable for meditation and relaxation guidance.",
	});
...
async function streamingTTS() {
	const response = await fetch("https://api.openai.com/v1/audio/speech", {
		...
		body: JSON.stringify({
			...
			voice: "nova",
			input:
				"This is a streaming audio example. The audio will be generated and delivered in chunks.",
...
async function withErrorHandling() {
	try {
		...
		if (error.message.includes("input too long")) {
			...
			console.error(
				"Voice not recognized - use one of the 11 supported voices",
			);
