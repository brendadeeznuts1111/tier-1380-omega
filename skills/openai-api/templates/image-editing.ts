...
async function basicEdit() {
	...
	formData.append(
		"prompt",
		"Change the sky to a sunset with orange and pink colors",
	);
...
async function formatComparison() {
	...
	return {
		png: pngData.data[0].url,
		jpeg: jpegData.data[0].url,
		webp: webpData.data[0].url,
	};
}
...
async function commonEdits() {
	...
	colorCorrect.append(
		"prompt",
		"Increase brightness and saturation, make colors more vibrant",
	);
	...
	addText.append(
		"prompt",
		'Add the text "SALE" in large bold letters at the top',
	);
