...
const tableData = [
	[
		{
			text: "Metric",
			options: { bold: true, fill: "0066CC", color: "FFFFFF" },
		},
		{
			text: "Q1 2025",
			options: { bold: true, fill: "0066CC", color: "FFFFFF" },
...
const steps = [
	...
	{
		num: "2",
		text: "Finalize expansion strategy for APAC region",
		color: "00AA44",
	},
...
if (isMainModule) {
	saveToFile("quarterly-report.pptx").then(() => {
		...
		console.log(
			"   Slides: 6 (Title, Highlights, Table, Line Chart, Bar Chart, Next Steps)",
		);
	});
}
