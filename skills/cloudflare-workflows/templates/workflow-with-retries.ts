	...
	async run(event: WorkflowEvent<PaymentParams>, step: WorkflowStep) {
			...
			async () => {
				if (!orderId || !customerId) {
					throw new NonRetryableError(
						"Missing required fields: orderId or customerId",
					);
				}
		...
		try {
			paymentResult = await step.do(
				...
				async () => {
					const response = await fetch(
						"https://primary-payment-gateway.example.com/charge",
						{
							method: "POST",
					...
					if (!response.ok) {
						...
						if (response.status === 401 || response.status === 403) {
							throw new NonRetryableError(
								"Authentication failed with payment gateway",
							);
						}
			...
			paymentResult = await step.do(
				...
				async () => {
					const response = await fetch(
						"https://backup-payment-gateway.example.com/charge",
						{
							method: "POST",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify({
		...
		try {
			await step.do(
				...
				async () => {
					await fetch(
						"https://api.example.com/notifications/payment-confirmed",
						{
							method: "POST",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify({
