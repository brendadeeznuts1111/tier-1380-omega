...
async function testConnection() {
	...
	if (!connectionString) {
		...
		console.log(
			'  export DATABASE_URL="postgresql://user:pass@ep-xyz-pooler.region.aws.neon.tech/db?sslmode=require"',
		);
		process.exit(1);
	}
	...
	if (connectionString.includes("-pooler.")) {
		console.log(
			"✅ Using pooled connection string (recommended for serverless)",
		);
	} else {
		console.log("⚠️  Warning: Not using pooled connection string");
		console.log(
			"   For serverless, use pooled connection: ...@ep-xyz-pooler.region.aws.neon.tech/...",
		);
	}
	...
	if (connectionString.includes("sslmode=require")) {
		...
		console.log(
			'⚠️  Warning: SSL mode not set. Add "?sslmode=require" to connection string',
		);
	}
    ...
    `;
		...
		const selectResult =
			await sql`SELECT * FROM connection_test ORDER BY created_at DESC LIMIT 1`;
		...
		console.log("");
		console.log(
			"🎉 All tests passed! Your Neon/Vercel Postgres connection is working correctly.",
		);
	} catch (error: any) {
