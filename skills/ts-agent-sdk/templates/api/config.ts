...
export function loadAPIConfig(): APIConfig {
	return {
		...
		workersAI: {
			accountId:
				process.env.SDK_WORKERS_AI_ACCOUNT_ID ||
				process.env.SDK_CF_ACCOUNT_ID ||
				"",
			apiToken:
				process.env.SDK_WORKERS_AI_API_TOKEN ||
				process.env.SDK_CF_API_TOKEN ||
				"",
		},
		slack: {
...
export function validateWorkersAIConfig(config: APIConfig): void {
	if (!config.workersAI.accountId) {
		throw new Error(
			"SDK_WORKERS_AI_ACCOUNT_ID is required for Workers AI calls.",
		);
	}
	if (!config.workersAI.apiToken) {
		throw new Error(
			"SDK_WORKERS_AI_API_TOKEN is required for Workers AI calls.",
		);
	}
}
...
export function validateSlackConfig(config: APIConfig): void {
	if (!config.slack.webhookUrl) {
		throw new Error(
			"SDK_SLACK_WEBHOOK_URL is required for Slack notifications.",
		);
	}
}
