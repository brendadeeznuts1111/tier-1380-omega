...
export class DBClient {
	...
	private async executeLocalRaw<T>(
		sql: string,
		params?: SQLParam[],
	): Promise<D1QueryResult<T>> {
		...
		try {
			const output = execSync(
				`npx wrangler d1 execute ${dbName} --local --json --command='${finalSql}'`,
				{
					encoding: "utf-8",
	...
	private async executeRemoteRaw<T>(
		sql: string,
		params?: SQLParam[],
	): Promise<D1QueryResult<T>> {
