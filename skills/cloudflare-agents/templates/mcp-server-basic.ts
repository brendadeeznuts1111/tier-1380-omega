...
export class StatefulMCP extends McpAgent<Env, CounterState> {
	...
	async init() {
		...
		this.server.resource("history", "mcp://resource/history", (uri) => ({
			contents: [
				{
					uri: uri.href,
		...
		// Tool: Reset counter
		this.server.tool("reset", "Reset the counter to zero", {}, async () => {
			this.setState({
				counter: 0,
...
// Stateful MCP server
app.mount("/stateful/mcp", StatefulMCP.serve("/stateful/mcp").fetch, {
