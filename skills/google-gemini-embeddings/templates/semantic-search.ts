...
async function main() {
	try {
		...
		results1.forEach((result, i) => {
			console.log(
				`${i + 1}. [${(result.similarity * 100).toFixed(1)}%] ${result.document.id}`,
			);
		...
		console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
		console.log(
			'Query: "neural networks" (no exact keyword match in any document)\n',
		);
		...
		results3.forEach((result, i) => {
			...
			console.log(
				`${i + 1}. [${(result.similarity * 100).toFixed(1)}%] ${result.document.id} ${hasKeyword ? "✓ keyword" : "✗ no keyword"}`,
			);
			console.log(`   ${result.document.text}\n`);
		});
