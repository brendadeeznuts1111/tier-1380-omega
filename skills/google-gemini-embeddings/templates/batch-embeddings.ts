...
class EmbeddingService {
	...
	async embedText(
		...
		const { taskType = "SEMANTIC_SIMILARITY", outputDimensionality = 768 } =
			options;
		...
		for (let attempt = 0; attempt < this.config.maxRetries; attempt++) {
			try {
				...
				if (error.status === 429 && !isLastAttempt) {
					const delay = this.config.initialDelayMs * 2 ** attempt;
					console.log(
						`⚠️ Rate limit error. Retrying in ${delay / 1000}s... (attempt ${attempt + 1}/${this.config.maxRetries})`,
					);
	...
	async embedBatch(
		...
		for (let i = 0; i < texts.length; i++) {
			...
			if ((i + 1) % 10 === 0 || i === texts.length - 1) {
				...
				console.log(
					`✅ ${i + 1}/${texts.length} (${rate.toFixed(1)} texts/sec, ETA: ${eta.toFixed(1)}s)`,
				);
			}
		}
	...
	async embedBatchAPI(
		...
		const { taskType = "RETRIEVAL_DOCUMENT", outputDimensionality = 768 } =
			options;
...
async function main() {
	try {
		...
		console.log(
			`✨ Completed in ${elapsed.toFixed(1)}s (${documents.length} texts in 1 request)\n`,
		);
