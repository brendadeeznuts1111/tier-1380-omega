...
function kMeansClustering(
	...
	console.log(
		`🔄 Starting K-means clustering (k=${k}, max iterations=${maxIterations})\n`,
	);
	...
	while (iteration < maxIterations && !converged) {
		...
		const clusters: Document[][] = Array(k)
			.fill(null)
			.map(() => []);
		...
		clusters.forEach((cluster, i) => {
			...
			const newCentroid = cluster[0].embedding!.map(
				(_, dim) =>
					cluster.reduce((sum, doc) => sum + doc.embedding![dim], 0) /
					cluster.length,
			);
	...
	const finalClusters: Cluster[] = centroids.map((centroid, i) => ({
		...
		documents: documents.filter((doc) => {
			const similarities = centroids.map((c) =>
				cosineSimilarity(doc.embedding!, c),
			);
...
function printClusters(clusters: Cluster[], method: string): void {
	...
	clusters.forEach((cluster) => {
		console.log(
			`📁 Cluster ${cluster.id + 1} (${cluster.documents.length} documents):`,
		);
		console.log(`${"─".repeat(50)}`);

		cluster.documents.forEach((doc) => {
			const preview =
				doc.text.substring(0, 80) + (doc.text.length > 80 ? "..." : "");
...
async function main() {
	try {
		...
		const documents: Document[] = [
			...
			{
				id: "doc3",
				text: "Rome is the capital of Italy and famous for the Colosseum and Vatican City.",
			},
			...
			{
				id: "doc7",
				text: "Pizza originated in Italy and is now popular worldwide. It typically has a tomato base and cheese.",
			},
			{
				id: "doc8",
				text: "Sushi is a Japanese dish made with vinegared rice and various ingredients like raw fish.",
			},
			{
				id: "doc9",
				text: "Tacos are a traditional Mexican food consisting of a tortilla filled with various ingredients.",
			},
		];
		...
		printClusters(
			thresholdClusters,
			"Threshold-Based Clustering (≥70% similarity)",
		);
		...
		kMeansClusters.forEach((cluster) => {
			...
			console.log(
				`  Documents: ${cluster.documents.map((d) => d.id).join(", ")}`,
			);
