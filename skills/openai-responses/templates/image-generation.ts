...
async function imageForPresentation() {
	...
	const response = await openai.responses.create({
		model: "gpt-5",
		input:
			"Create a professional infographic showing the growth of AI adoption from 2020 to 2025",
		tools: [{ type: "image_generation" }],
	});
...
async function combinedImageAndAnalysis() {
	...
	const response = await openai.responses.create({
		model: "gpt-5",
		input:
			"Create a chart showing sales growth from 2020-2025, then generate an image visualization",
