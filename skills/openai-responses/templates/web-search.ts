...
async function researchQuestion() {
	...
	const response = await openai.responses.create({
		model: "gpt-5",
		input:
			"What are the pros and cons of using Cloudflare Workers for serverless applications?",
		tools: [{ type: "web_search" }],
	});
...
async function comparisonResearch() {
	...
	const response = await openai.responses.create({
		model: "gpt-5",
		input:
			"Compare the features and pricing of OpenAI GPT-5 vs Anthropic Claude 3.5 Sonnet",
		tools: [{ type: "web_search" }],
	});
...
async function productReviews() {
	...
	const response = await openai.responses.create({
		model: "gpt-5",
		input:
			"What are people saying about the iPhone 16 Pro? Find recent reviews.",
		tools: [{ type: "web_search" }],
	});
...
async function combinedTools() {
	...
	const response = await openai.responses.create({
		model: "gpt-5",
		input:
			"Find the current Bitcoin price and calculate what $1000 would be worth",
...
async function webSearchWithFileSearch() {
	...
	const file = await openai.files.create({
		file: Buffer.from(
			"Internal policy: Always check external sources for pricing info",
		),
		purpose: "assistants",
	});
