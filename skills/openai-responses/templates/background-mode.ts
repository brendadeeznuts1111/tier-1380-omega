...
async function backgroundWithCodeInterpreter() {
	...
	const response = await openai.responses.create({
		model: "gpt-5",
		input:
			"Process this large dataset and generate detailed statistical analysis",
		background: true,
...
async function backgroundWithFileSearch() {
	...
	const response = await openai.responses.create({
		model: "gpt-5",
		input:
			"Read this entire document and extract all key insights, metrics, and action items",
		background: true,
...
async function backgroundWithWebSearch() {
	...
	const response = await openai.responses.create({
		model: "gpt-5",
		input:
			"Research the top 50 AI companies and create a comprehensive comparison report",
		background: true,
