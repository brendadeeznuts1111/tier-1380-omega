...
async function chatWithGrounding() {
	...
	response = await chat.sendMessage(
		"Which company made the biggest breakthrough?",
	);
	console.log(`\nAssistant: ${response.text}`);
...
async function groundingWithFunctionCalling() {
	...
	console.log(
		"User query: What is the weather like in the city that won Euro 2024?",
	);
	console.log("\nModel will:");
...
async function checkingGroundingUsage() {
	...
	console.log(
		`Grounding used: ${response2.candidates[0].groundingMetadata ? "YES" : "NO"}`,
	);

	if (response2.candidates[0].groundingMetadata) {
		console.log("\nSearch queries performed:");
		const queries =
			response2.candidates[0].groundingMetadata.searchQueries || [];
		queries.forEach((q, i) => {
