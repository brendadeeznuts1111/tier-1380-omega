...
async function main() {
	...
	try {
		...
		console.log(
			"User: What is the weather, population, and timezone of Tokyo?\n",
		);
		...
		const functionCalls =
			response1.candidates[0]?.content?.parts?.filter(
		...
		const functionResponses = await Promise.all(
			functionCalls.map(async (part) => {
				...
				console.log(
					`- Calling ${functionCall.name} with args:`,
					functionCall.args,
				);
		...
		const response2 = await ai.models.generateContent({
			...
			contents: [
				{
					parts: [
						{ text: "What is the weather, population, and timezone of Tokyo?" },
					],
				},
