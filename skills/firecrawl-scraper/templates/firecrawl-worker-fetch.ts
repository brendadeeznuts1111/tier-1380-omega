 ...
 * Check crawl status
 */
async function checkCrawlStatus(crawlId: string, apiKey: string): Promise<any> {
	...
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		if (request.method !== "POST") {
			return Response.json(
				{
					error:
						'Method not allowed. Use POST with JSON body: { "url": "..." }',
				},
		...
		try {
			...
			if (!env.FIRECRAWL_API_KEY) {
				return Response.json(
					{
						error:
							"FIRECRAWL_API_KEY not configured. Run: npx wrangler secret put FIRECRAWL_API_KEY",
					},
