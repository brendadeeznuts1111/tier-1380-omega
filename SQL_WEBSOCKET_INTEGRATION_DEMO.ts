...
export class OMEGAProfileManager {
	 ...
	 */
	generateProfileInsertSQL(profile: ProfileData): {
		sql: string;
		values: any[];
	} {
	...
	batchInsertProfiles(
		...
		const queries = completeProfiles.map((profile) =>
			this.generateProfileInsertSQL(profile),
		);

		console.log(
			`📦 Generated ${queries.length} SQL INSERT statements with undefined value filtering`,
		);
		return queries;
	}
		...
		scenarios.forEach((scenario) => {
			...
			try {
				const ws = this.createWebSocketConnection(
					"stream/profiles",
					this.getProxyConfigurations()[scenario.type],
				);
	...
	generateSQLExamples(): void {
		...
		console.log(
			"✅ team and benchmark omitted - database DEFAULT will be used\n",
		);
		...
		console.log(
			"✅ All undefined values filtered out - database DEFAULTs used\n",
		);
	...
	runIntegrationDemo(): void {
		...
		console.log(
			`📊 Created ${sampleProfiles.length} sample profiles for streaming`,
		);
