...
import {
	type ProfileUrl,
	profilePublisher,
} from "../profile/ProfilePublisher.ts";
...
export class MatrixRegistry {
	...
	private getOwnerForColumn(column: number): string {
		...
		if (team === "compiler")
			return `compiler-lead-${Math.ceil((column - 20) / 5)}`;
	...
	queryCellsWithProfiles(options?: {
		...
		this.grid.columns.forEach((col) => {
			col.cells.forEach((cell) => {
				...
				if (
					options?.profileType &&
					cell.profileUrl.metadata.type !== options.profileType
				)
					return;
				if (
					options?.environment &&
					cell.profileUrl.metadata.environment !== options.environment
				)
					return;
		...
		this.grid.columns.forEach((col) => {
			col.cells.forEach((cell) => {
				if (cell.profileUrl) {
					...
					stats.profilesByTeam[cell.team] =
						(stats.profilesByTeam[cell.team] || 0) + 1;

					const env = cell.profileUrl.metadata.environment;
					stats.profilesByEnvironment[env] =
						(stats.profilesByEnvironment[env] || 0) + 1;
				}
			});
		});
