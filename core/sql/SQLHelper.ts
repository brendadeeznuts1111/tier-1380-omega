...
export class SQLHelper {
  ...
  static sql(strings: TemplateStringsArray, ...values: any[]): SQLQuery {
    ...
    for (let i = 0; i < strings.length; i++) {
      ...
      if (i < values.length) {
        ...
        if (this.isInsertObject(value)) {
          const insertQuery = this.buildInsertQuery(value, paramIndex);
          text += insertQuery.text;
          params.push(...insertQuery.values);
          paramIndex += insertQuery.values.length;
        } else if (this.isInsertArray(value)) {
          const bulkInsertQuery = this.buildBulkInsertQuery(value, paramIndex);
  ...
  private static isInsertObject(value: any): value is InsertData {
    return value && typeof value === 'object' && !Array.isArray(value) && !this.isSQLQuery(value);
  }
  ...
  private static isSQLQuery(value: any): value is SQLQuery {
    return value && typeof value === 'object' && 'text' in value && 'values' in value;
  }
  ...
  static buildInsertQuery(data: InsertData, startParamIndex: number): SQLQuery {
    ...
    const columns = definedEntries.map(([key]) => this.quoteIdentifier(key));
    const placeholders = definedEntries.map((_, index) => `$${startParamIndex + index}`);
  ...
  static buildBulkInsertQuery(dataArray: InsertData[], startParamIndex: number): SQLQuery {
    ...
    const quotedColumns = columnList.map(col => this.quoteIdentifier(col));
  ...
  static buildUpdateQuery(table: string, data: InsertData, whereClause: string, whereValues: any[] = []): SQLQuery {
    ...
    const setClauses = definedEntries.map(([key, _], index) => 
    ...
    const text = `UPDATE ${this.quoteIdentifier(table)} SET ${setClauses.join(', ')} ${whereClause}`;
  ...
  static buildSelectQuery(
    ...
    let text = `SELECT ${quotedColumns.join(', ')} FROM ${this.quoteIdentifier(table)}`;
  ...
  static buildDeleteQuery(table: string, whereClause: string, whereValues: any[] = []): SQLQuery {
    const text = `DELETE FROM ${this.quoteIdentifier(table)} ${whereClause}`;
    return { text, values: whereValues };
  }
   ...
   */
  static buildWhereClause(filters: InsertData, operator: 'AND' | 'OR' = 'AND'): { clause: string; values: any[] } {
    const definedEntries = Object.entries(filters).filter(([_, value]) => value !== undefined);
    
    if (definedEntries.length === 0) {
      return { clause: '', values: [] };
    }

    const conditions = definedEntries.map(([key, _], index) => 
      `${this.quoteIdentifier(key)} = $${index + 1}`
    );
  ...
  static validateQuery(query: SQLQuery): boolean {
    ...
    if (paramCount !== query.values.length) {
      console.warn(`Parameter count mismatch: ${paramCount} placeholders, ${query.values.length} values`);
      return false;
    }
...
export const OMEGASQLExamples = {
  ...
  updateProfile: (id: string, data: SQLInsert) => {
    ...
    const updateQuery = SQLHelper.buildUpdateQuery('profiles', data, clause, values);
