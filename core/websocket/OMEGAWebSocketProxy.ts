...
export const OMEGAWebSocket = {
  ...
  connectProfileStream: (proxyUrl: string) => {
    ...
    return OMEGAWebSocketProxy.createOMEGAProfileWebSocket(
      'stream/profiles',
      { url: proxyUrl }
    );
  },
   ...
   */
  connectAuthenticated: (wsUrl: string, proxyUrl: string, credentials: { username: string; password: string }) => {
  ...
  testProxy: async (wsUrl: string, proxyUrl: string) => {
    ...
    const result = await OMEGAWebSocketProxy.testProxyConnection(wsUrl, proxyUrl);
    
    if (result.success) {
      console.log(`✅ Proxy connection successful (${result.latency}ms latency)`);
    } else {
  ...
  createResilientConnection: (
    ...
    const ws = OMEGAWebSocketProxy.createWebSocket(wsUrl, { proxy: proxyOptions });
