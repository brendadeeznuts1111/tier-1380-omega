...
export class DataFlowPipeline {
  ...
  private extractTeamFromCategory(category: string): string {
    ...
    if (categoryLower.includes('runtime') || categoryLower.includes('performance')) {
      return 'runtime';
    }
    if (categoryLower.includes('compiler') || categoryLower.includes('build')) {
      return 'compiler';
    }
    if (categoryLower.includes('platform') || categoryLower.includes('system')) {
      return 'platform';
    }
  ...
  private enhanceContent(description: string, team: string): string {
    const teamContext = {
      runtime: 'This runtime optimization impacts Air IR performance and buffer management.',
      compiler: 'This compiler enhancement affects code generation and optimization passes.',
      platform: 'This platform improvement influences boot time and system integration.'
    };
  ...
  private extractTagsFromRSS(rssItem: RSSItem): string[] {
    ...
    const keywords = ['performance', 'optimization', 'memory', 'speed', 'air', 'ir', 'buffer'];
  ...
  private getColumnForTeam(team: string): number {
    const mapping = this.teamColumnMapping[team as keyof typeof this.teamColumnMapping];
   ...
   */
  private getProfileTypeForTeam(team: string): 'cpu' | 'heap' | 'tension' | 'memory' | 'performance' {
    const mapping: Record<string, 'cpu' | 'heap' | 'tension' | 'memory' | 'performance'> = {
      runtime: 'cpu',
      compiler: 'tension',
      platform: 'memory'
    };
  ...
  async processRSSBatch(items: RSSItem[]): Promise<{
    ...
    for (const item of items) {
      try {
        ...
        // Update summary
        summary.byTeam[result.blogPost.team] = (summary.byTeam[result.blogPost.team] || 0) + 1;
